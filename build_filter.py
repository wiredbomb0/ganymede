...
def PrintColors():
  for key, value in color_swap.iteritems():
    ...
    if "LIGHT" in key:
      light = key
      dark  = key.replace("_LIGHT", "_DARK")

      light = color_swap[light].replace(" ", ",")
      dark  = color_swap[dark].replace(" ", ",")
      text  = key.replace("_LIGHT", "")

      print '<span style="background-color: rgb({}); color: rgb({}); border: 1px solid rgb({})">{}</span><br>'.format(dark, light, light, text)
